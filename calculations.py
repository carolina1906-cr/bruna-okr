def calcular_avance(kr, valores_mes, mes_activo, meta_progresiva=None):
    tipo = kr["measurement_type"]
    delta = kr["delta"]
    meta = meta_progresiva if meta_progresiva is not None else kr["goal"]
    ...
    if tipo == "completado":
        ...
        if not vals:
            return pct_m, None
        acum = sum(vals)
        pct_a = pct_simple(acum, kr["goal"], delta, base)
        return pct_m, pct_a
